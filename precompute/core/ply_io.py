...
# --- SH degree-0 helper (never used for export albedo, only for naive init) ---
...
# NOTE: GDGS additionally *centers* imported data and applies a default -180 deg
# Z-correction on new GaussianSplatNodes. Reconcile that at export time (M1) — see
# docs/decisions.md — do NOT compensate for it Godot-side.
...
def _parse_header(f):
    ...
    (props:list[(name,np_type)], n_vertex:int, data_offset:int)."""
    line = f.readline()
    if line.strip() != b"ply":
    ...
    while True:
        ...
        if kw == b"element":
            ...
            break
    return props, n_vertex, f.tell()


def _read_structured(path: str):
    with open(path, "rb") as f:
        props, n, offset = _parse_header(f)
...
def read_asset_ply(path: str) -> AssetGaussians:
    arr, names = _read_structured(path)
...
def read_standard_3dgs_ply(path: str) -> dict:
    ...
    missing = {"x", "opacity", "scale_0", "rot_0"} - nameset
    if missing:
        raise ValueError(f"not a standard 3DGS PLY, missing {missing}")
    return out
