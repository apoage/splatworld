...
| D6 | normal-sign efficacy on real foliage: does the scheduled re-decompose reach multi-scale sign-opposition <5% on both heroes, or does the along-view camera-hemisphere approach need a SCALABLE grazing-normal sign resolver? | RUN the scheduled re-decompose FIRST — the always-on fail-closed gate decides objectively. If it fails-closed (>5%), camera-orientation alone is insufficient → pick a SCALABLE grazing-normal resolver (visibility-weighted orientation accumulated during the solve / anisotropy-gated flipping / coarse-voxel sign field), NOT the test-only MST BFS (doesn't scale to millions of Gaussians). | v0.16.0 shipped the infra + fail-closed gate (verified). Correctness re-verify (synthetic): orient+smooth×2 leaves 17–49% opposition at hero coherence with realistic grazing/away-facing normals (in-loop hard fixture recorded 21.6/22.2%); camera-orient resolves only the along-view component. Gate is fail-closed so a marginal asset REFUSES to ship (owner keeps patch shadows) — informative, not harmful. Blocks the normal-sign owner-facing acceptance + M3 backlit dot(−N,L). |
