...
|---|----------|----------------|---------|
| D4 | Wire the recovered environment light (`env_sh.json` sidecar) into the Godot runtime's ambient term? | **YES** — replaces M2a's flat ambient constant with the scene lighting decompose actually recovered, completing the relight thesis (currently the sidecar is written but unread). Small coordinated step: Godot `ambient_sh(N)` reader sharing `core/sh_env.py` constants; NO per-Gaussian schema change (SCHEMA_VERSION stays 1). Sequence AFTER M2b phase D (real assets produce the first meaningful sidecar). Task ready: `tasks/2026-07-12-env-sh-runtime.md`. | factory run #2 wrap-up; decisions.md 2026-07-12 M2 entry |
