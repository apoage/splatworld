...
| — | **recurring-quality-pass slice 5 (NOW UNBLOCKED)** | S–M | **Roll out the D5 fix to the viewer assets** (the fix is default-OFF, so the sparkle the owner saw is still in the mirrored asset). Promote `.perf/normalsmooth/pxl_144634/` (validated smoothed decompose, coherence 0.922) → `assets/built` + re-export + re-mirror to `gs_assets`; re-decompose pxl_131945 with `--smooth-normals-iters 2`; then regen demo/gif. Highest user-visible value. |
