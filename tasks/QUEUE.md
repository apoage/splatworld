...
| 1 | `tasks/2026-07-15-relit-energy.md` | S–M | **TOP — owner-visible shading bug, diagnosis CONFIRMED with data in the task file: env-SH ambient applied at weight 1.0 (4.2–4.4× the design 0.2) → 23–27% of splats clip → "bloom with extra saturation" + capture-light "made-up shadows". Fix = DC-normalize at set_env_sh + scale by the ambient slider.** Do BEFORE flashlight-orb (same shader seam; calibrate energy first, then add lights) |
| 2 | `tasks/2026-07-15-normal-sign-consistency.md` | M | **Owner-visible defect #2, audit-confirmed: ~29% of neighbor normals sign-opposed in decompose output; the sign-naive D5 smoothing coarsened them into randomly-signed DOMAINS (~0.05–0.1 units) = the patchy fake shadows under direct sun. Fix = sign-consistency pass at the source + sign-aware smoothing + folded-coherence metric.** M3 prerequisite (backlit term needs signs). Full audit numbers in the task file |
| 3 | `tasks/2026-07-15-flashlight-orb.md` | M | **Owner, post-D5-eyeball 2026-07-15: flashlight (camera-attached point/spot in the relight pass — the Moon-Stone fireball prerequisite pulled forward) + engine-lit reference orb toggle.** Shading-contract change — high-tier verification. FPS HUD + V/A/D diagnostic toggles already shipped planner-side 2026-07-15; orb reuses render_matrix's sphere_consistency placement helper |
