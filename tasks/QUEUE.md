...
| ~~—~~ | ~~**quality-pass slice 6 (aesthetic splat cleanup)**~~ | S | ✅ **SUPERSEDED + DONE (2026-07-24) by the SuperSplat round-trip** — instead of export's prune flags, the owner cleaned both heroes interactively in SuperSplat (pxl_144634 −42%, pxl_131945 −50%), then re-decomposed via the v0.27.0 baseline-refresh path → cleaned relightable `asset.vply`. Owner eyeball: "gorgeous" (2026-07-24, DECISIONS D7 note f). Masked PSNR healthy (20.7/23.3); full-frame drop = background pruned, object intact. `9f7b5ec`. |
