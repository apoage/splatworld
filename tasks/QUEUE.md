...
| 1 | `tasks/2026-07-17-sign-agnostic-prototype.md` | S–M | **TOP (owner 2026-07-17 "prototype it in factory"): D7 shading experiment behind a `sign_mode` toggle — modes: signed (default, byte-identical) / two-lobe abs / flip-toward-camera / confidence-blend (stretch). Viewer key `N` cycles live; owner A/B in sun-only mode = the D7 decision evidence. Runtime-only, default OFF, mode-0 regression gate + per-mode analytic fault-injection checks. Does NOT decide D7 — the owner's eyeball does. Companion research prompt (owner runs it): `prototype/research-prompt-d7-sign-shading.md`** |
