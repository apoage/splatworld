...
static func create_push_constant(data: Array) -> PackedByteArray:
	...
	# Godot 4.7 requires the push-constant upload size to match the shader's
	# reflected push_constant block size EXACTLY (older Godot rounded up to 16).
	# Supply exactly packed_size bytes; callers size their arrays to match.
