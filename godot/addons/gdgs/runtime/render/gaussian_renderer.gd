...
func _rasterize_state(state, point_count: int) -> void:
	...
	for radix_shift_pass in range(4):
		...
		var spine_pc := RenderingDeviceContext.create_push_constant([radix_shift_pass])
		var down_pc := RenderingDeviceContext.create_push_constant([radix_shift_pass, in_offset, out_offset])
		state.pipelines["radix_sort_upsweep"].call(state.context, compute_list, up_pc, [], state.descriptors["grid_dimensions"].rid, 0)
		state.pipelines["radix_sort_spine"].call(state.context, compute_list, spine_pc)
		state.pipelines["radix_sort_downsweep"].call(state.context, compute_list, down_pc, [], state.descriptors["grid_dimensions"].rid, 0)
	state.context.compute_list_end()
